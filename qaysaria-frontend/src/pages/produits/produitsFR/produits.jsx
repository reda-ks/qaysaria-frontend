...
import { useSearchParams, useNavigate } from 'react-router-dom'; // ← ajout useNavigate
...
const Produits = () => {
  ...
  const filteredData = useMemo(() => {
    return allProducts.filter((p) => {
      ...
      if (filters.audiences && filters.audiences.length > 0) {
        const audienceMap = { '1': 'Femme', '2': 'Homme', '3': 'Enfant', '4': 'Bébé' };
  ...
  const handleWhatsApp = (product) => {
    const message = encodeURIComponent(
      `Bonjour ! L'article *${product.name}* est-il disponible ?`
    );
    ...
    <div className="produits-page">
      ...
      <div className="produits-layout">
        ...
        <div className="produits-main">
          ...
          {loading ? (
            <div className="produits-status-box">
              <h3>Chargement des pépites...</h3>
            </div>
            ...
            <div className="produits-grid">
              {paginatedProducts.map((product) => (
                <div
                  key={product.id}
                  className="p-card"
                  onClick={() => handleProductClick(product.id)} // ← ajout
